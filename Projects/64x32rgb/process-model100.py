...
import sys
if (len (sys.argv) != 2):
    print ("Usage: python3 process-model100.py <output>")
    sys.exit (1)

# [' '] = {0x00,0x00,0x00,0x00,0x00,0x00},
font = [i.strip () for i in fontstr.splitlines () if i.strip ()]

arr = bytearray (5 * len (font))
for i in font:
    index = ord (i [2]) - ord (' ')
    data = bytes.fromhex (i [9 : -2].replace (',', '').replace ('0x', '')) [1 : ] # first byte is 0x00

    # change from vertical to horizontal matrix
    data = ["1" if (c & (1 << b)) else "0" for b in range (8) for c in data]
    data = bytes (int ("".join (data [i * 8 : (i + 1) * 8]), 2) for i in range (5))

    arr [index * 5 : (index + 1) * 5] = data
